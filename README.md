...
- **Evidence Locker** — Focus view for source documents; switch entities to open the PDF viewer with real PDF rendering, or stay in `ALL` to work the Claimed vs. Proven schedule with Manual / Import / Auto-Calc entry modes. Analysts can add/edit/delete manual expense lines, reorder the schedule with up/down controls, spin up new categories inline, and resize both left-hand and right-hand panes via drag handles.
